"""
crowdclassicd JSONRPC interface
"""
import sys
import os
...
class CRowdCLassicDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some crowdclassicd methods
    ...
    @classmethod
    def from_crowdclassic_conf(self, crowdclassic_dot_conf):
        from crowdclassic_config import CRowdCLassicConfig
        config_text = CRowdCLassicConfig.slurp_config_file(crowdclassic_dot_conf)
        creds = CRowdCLassicConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = crowdclassiclib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = crowdclassiclib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = crowdclassiclib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_crowdclassicd_proto_version_with_sentinel_ping)
