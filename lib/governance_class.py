...
class GovernanceClass(object):
    ...
    def vote(self, crowdclassicd, signal, outcome):
        return self.go.vote(crowdclassicd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, crowdclassicd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(crowdclassicd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(crowdclassicd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to crowdclassicd "gobject list" output
    ...
    def crowdclassicd_serialise(self):
        import crowdclassiclib
        return crowdclassiclib.SHIM_serialise_for_crowdclassicd(self.serialise())
